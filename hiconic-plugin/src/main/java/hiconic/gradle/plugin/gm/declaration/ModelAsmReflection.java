...
public class ModelAsmReflection implements ModelReflection {
	...
	public ModelAsmReflection(Project project, ClassLoader classloader) {
	...
	public static ModelReflection scan(Project project, ClassLoader classLoader) {
		ModelAsmReflection mar = new ModelAsmReflection(project, classLoader);
		return mar;
	}
	...
	protected boolean isEnum(String superType) {
		return acquireEntity(superType).getIsEnum();
	}
	...
	protected boolean isGenericEntity(String interfaceName) {
		...
		return acquireEntity(interfaceName).getIsGenericEntity();
	}

	private Entity acquireEntity(String interfaceName) {
		Entity en = nameToEntityMap.get(interfaceName);
		if (en == null)
			en = load(interfaceName);

		if (en == null)
			throw new GradleException("Cannot analyze the model, type not found on classpath: [" + interfaceName.replace('/', '.')
					+ "]. It is a super type of a type of this model (project: " + project.getName() + ").");
		return en;
	}
